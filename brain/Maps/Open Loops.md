...
## 2026-06-23 — VERCEL DEPLOY QUOTA EXHAUSTED (hard 24h block on frontend deploys)
PR #369 (admin referral overview) MERGED to prod (sha 3cb10d7) — backend endpoint live on Railway. But Vercel returned `api-deployments-free-per-day` ("more than 100, try again in 24 hours"): the session's 11 merged PRs (#360-#369), each triggering preview builds on 2 Vercel projects + draft→ready transitions, blew through the free-tier 100-deploys/day cap.
- **Effect:** ALL frontend deploys (preview + prod) fail for ~24h until reset. Frontend changes merged to `main` this session (SEO landing pages, AdminFunnelPage/TenantHealth/Referral, ReferralPage, signup ref capture) are committed + on main but may NOT be live on app.agentnexlify until the quota resets. Backend (Railway) + DB migrations + KB inserts are ALL live and unaffected.
- **This is the natural stop point for the autonomous loop:** shipping more frontend-touching PRs now produces undeployable changes and burns quota that's already capped. The highest-value action is to STOP and let the Vercel quota reset. Backend-only work could continue, but the remaining backlog is owner-gated (referral incentive) or low-marginal (more verticals). Resume frontend work after the 24h reset; owner can also upgrade Vercel to Pro to lift the cap.
- Loop total this session: 11 PRs merged to prod across product activation, conversion, acquisition (SEO + referral), retention (churn-watch + digest), analytics (3 admin dashboards), data-quality, kill-trial, 13-vertical moat, and end-to-end referral attribution.

## Review 2026-06-23 (round 14, backend-only — Vercel cap workaround) — referral signup notification
With frontend deploys quota-blocked, continued the loop on backend-only (Railway) work. `notify_referrer_of_signup` (in `referral.py`): when a referred signup is attributed, resolve referrer via `referred_by_widget_key`→`widget_configs.api_key`→`owner_email` and email them "someone signed up through your link" + a link to /dashboard/referral. Hooked into the register flow at `auth.py:359` via `safe_create_task` (fire-and-forget, non-blocking; graceful skip on missing/faked ref; swallowed send failures). Reuses existing mailer. 10 notify tests + auth-register regression green. Activates the referral channel socially (positive feedback → more sharing) — complements the click/signup tracking + admin dashboard. Backend-only → deploys to Railway despite the Vercel frontend cap.
- **MERGED to prod: PR #370 (sha 69e6789).** 12th PR; backend-only Railway deploy.

## Review 2026-06-23 (round 15, backend-only) — referral stats in the weekly value digest
Cross-pollinated the referral channel into the Friday value digest tenants already open. New `backend/services/weekly_referrals.py::compute_weekly_referrals(db, tenant_id, since)` — resolves the tenant's embed key (`widget_configs.api_key`), counts share-link clicks (`referral_clicks.ref_tenant_id`) + referred signups (`tenants.referred_by_widget_key`) in the same 7-day window the value recap uses. Fault-tolerant (zeros on failure, never raises); skips the count queries entirely when the tenant has no embed key. Wired into `scheduled_jobs_ext._build_and_send_digest` non-empty branch: renders a "People you referred: N" row when signups>0 (the money line — each is a new paying tenant), else "Referral link clicks: N" when clicks>0, else nothing (a zero row every week reads as nagging). Row sits before the "Top customer question" closer so the table still ends on the italic question. 14 new tests + 34 digest regressions green. Backend-only → Railway-deployable despite the Vercel frontend cap. Files: `backend/services/weekly_referrals.py` (new), `backend/services/automation/scheduled_jobs_ext.py`, `backend/tests/test_weekly_referrals.py` (new).
- **MERGED to prod: PR #371 (sha 6b1e41c).** 13th PR this session; backend-only Railway deploy. CI: PR Validation infra-death (job-log 404 = runner never allocated) + 2 Vercel rate-limit statuses (frontend quota, irrelevant to backend-only) — merged at `unstable` on the real gate (48 local tests green).
- **Backend-only high-value backlog now exhausted.** Remaining items are owner-gated (referral incentive/credit program — needs owner decision + Stripe credits) or low-marginal (more verticals). Frontend-touching work stays quota-blocked until the Vercel daily cap resets (~24h). Natural pause point for the loop.

## Review 2026-06-23 (post-loop, owner-directed) — audit + incentive design (non-shipping)
Loop paused shipping (Vercel cap + backend backlog exhausted). Owner picked: pause-until-reset + design referral incentive + architecture audit. Two non-deploying deliverables:
- **Architecture health audit** → `audits/audit-architecture-2026-06-23.md`. Counts: 0 CRITICAL · 5 HIGH · 8 MEDIUM · 6 LOW. **Schema-discipline CLEAN** (zero CRITICAL — `leads`/`conversations` use `client_id` everywhere; every `tenant_id` hit is on a table that legitimately uses it; `lead_stage`/`service_interest` are non-schema names). Top HIGH items: `widget_chat.py:313` ~16 blocking sync Supabase calls on the async `/chat` event loop (hottest path); `widget_chat.py` 1332-line + `onboarding.py` 1364-line god files; `schemas.py` 1010-line wide-blast-radius model file; lazy `python-dateutil` import only just pinned (same untracked-dep class as prior PyYAML/qrcode 500s). Fixes deferred to a separate session per the audit-then-fix-separately rule.
- **Referral incentive PRD** → `specs/referral-incentive_spec.md`. Build-ready design: reward referrer on referee's FIRST PAID invoice (fraud-resistant, self-funding) via Stripe customer balance credit; new `referral_rewards` table (idempotent, one per referee); reuses existing attribution + notify mailer + the `discounts`-at-checkout path (`billing.py:189`). **Blocked on 6 owner-only business decisions** (§6: reward amount, one/two-sided, trigger point, monthly cap, credit-vs-coupon mechanic, retroactive?). Backend grant path is Railway-deployable once decided; only the admin-dashboard row needs Vercel.
