...
class TestRecordingComplete:
    ...
    def test_recording_complete_stores_call(
        ...
        assert (
            fire_args[1]["event"] == "call.completed"
            or fire_args[0][1] == "call.completed"
        )
...
class TestTwimlHelpers:
    ...
    def test_build_twiml_greeting(self):
        from backend.routers.calls_webhooks import _build_twiml_greeting
    ...
    def test_build_twiml_error(self):
        from backend.routers.calls_webhooks import _build_twiml_error
            ...
            mock_client,
            {
                "chat_messages": (
                    [
                        {
                            "role": "assistant",
                            "content": "Thanks for calling Acme Plumbing!",
                        }
                    ],
                    1,
                ),
...
class TestGenerateCallSummary:
    ...
    async def test_summary_parses_json_response(
        self, mock_call_claude, mock_db, mock_activity
    ):
