...
class TestAnalyzeSEO:
    ...
    @patch("backend.services.local_seo_execute._generate_keywords")
    @patch("backend.routers.auth.settings")
    @patch("backend.services.local_seo_execute.get_service_supabase")
    def test_analyze_full_profile(self, mock_db, mock_settings, mock_keywords):
        ...
        assert data["keyword_suggestions"] == [
            "plumber in Austin",
            "emergency plumber Austin TX",
        ]
    ...
    @patch("backend.routers.auth.settings")
    @patch("backend.services.local_seo_execute.get_service_supabase")
    def test_analyze_updates_existing_profile(
        self, mock_db, mock_settings, mock_keywords
    ):
            ...
            if name == "tenants":
                ...
                results = iter(
                    [
                        ...
                        MagicMock(
                            data=[
                                {"id": "existing-profile-id", "tenant_id": "tenant-001"}
                            ],
                            count=1,
                        ),
                    ]
...
class TestGetSEOProfile:
    ...
    @patch("backend.routers.auth.settings")
    @patch("backend.services.local_seo_fetch.get_service_supabase")
...
class TestCompletenessCalculation:
    ...
    def test_perfect_score(self):
        from backend.services.local_seo_scoring import _calculate_completeness
