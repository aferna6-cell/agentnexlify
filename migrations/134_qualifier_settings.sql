-- Migration 134: per-tenant AI lead-qualifier settings
--
-- Item 4 of plans/non-technical-readiness-roadmap_plan.md. Adds an owner-facing
-- kill switch and a cost threshold for the AI lead qualifier
-- (backend/services/lead_qualification.py). Both columns are additive with
-- safe defaults so existing tenants keep current behavior:
--   qualifier_enabled    = true  -> qualifier runs (matches pre-134 behavior)
