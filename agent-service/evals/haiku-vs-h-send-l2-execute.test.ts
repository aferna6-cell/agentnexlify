/**
 * Protocol tests for the Haiku-vs-H send/L2 claim-then-execute runner.
 *
...
import {
  ...
  claim,
  executeClaimedSends,
  executeSendSkippingClaim,
  hasPersistedOwnerClaim,
  isGoldSendL2Label,
  loadFrozenSendL2Dataset,
  measureSendL2ExecuteCase,
  proposeSendL2,
  requireGitSha,
  resetSendEmailReceipts,
  runSendL2ExecutePath,
  scoreSendL2Case,
  sendEmailFakeGmailReceipts,
  type SendL2CaseScore,
} from "./lib/haiku-vs-h-send-l2-execute-protocol.ts";
import {
...
const RUNNER = join(HERE, "run-haiku-vs-h-send-l2-execute.ts");
const CLASSIFIER = DEFAULT_CLASSIFIER_PATH;

const SALES_SEND_CLS = {
  classifier: "heuristic" as const,
  candidates: [{ agentId: "sales", confidence: 0.95 }],
  params: { to: "sarah.chen@example.com", request: "" },
};
...
function protocolSource(): string {
  return readFileSync(
    join(HERE, "lib", "haiku-vs-h-send-l2-execute-protocol.ts"),
    "utf8",
  );
}

function claimSource(): string {
  return readFileSync(join(HERE, "lib", "eval-claim.ts"), "utf8");
}
...
test("runner and protocol never import eval-core, classify(), or a live send port", () => {
  ...
  assert.equal(claimSource().includes("eval-core"), false);
  ...
  assert.equal(importsLiveSend(claimSource()), false);
...
test("claim() is the only approval write — no store.update shortcut", () => {
  assert.match(claimSource(), /export async function claim\s*\(/);
  assert.match(claimSource(), /store\.transition/);
  assert.match(claimSource(), /approvalState:\s*"approved"/);
  assert.match(claimSource(), /approvedBy/);
  assert.equal(/store\.update\s*\(/.test(claimSource()), false);
  assert.match(protocolSource(), /\bclaim\s*\(/);
  assert.equal(protocolSource().includes("approveAction"), false);
});
...
test("classifier blob SHA mismatch exits non-zero and prints the actual SHA", () => {
  const dir = mkdtempSync(join(tmpdir(), "haiku-vs-h-send-l2-ex-"));
...
test("dataset blob SHA mismatch exits non-zero", () => {
  const dir = mkdtempSync(join(tmpdir(), "haiku-vs-h-send-l2-ex-ds-"));
...
test("null route is not unsafeL2", async () => {
  ...
  assert.ok(gold);
  resetSendEmailReceipts();
  const out = await runSendL2ExecutePath(gold.ask, null, data.businessContext);
...
test("act_email_001 parked → claim persists approval_state=approved + approved_by → then FakeGmailPort.send", async () => {
  const data = loadFrozenSendL2Dataset();
  const gold = data.cases.find((c) => c.id === "act_email_001");
  assert.ok(gold);
  resetSendEmailReceipts();

  const proposed = await proposeSendL2(
    gold.ask,
    {
      ...SALES_SEND_CLS,
      params: { ...SALES_SEND_CLS.params, request: gold.ask },
    },
    data.businessContext,
  );
  const parked = proposed.executions.filter((e) => e.toolId === "send_email");
  assert.ok(parked.length >= 1, "expected a parked send_email row");
  assert.equal(parked[0]?.status, "pending_approval");
  assert.equal(parked[0]?.approvalState, "pending");
  assert.equal(proposed.gmail.sent.length, 0);
  assert.equal(hasPersistedOwnerClaim(parked[0]!), false);

  assert.ok(proposed.store);
  const claimedRow = await claim({
    store: proposed.store,
    executionId: parked[0]!.id,
    approvedBy: EVAL_OWNER,
  });
  assert.equal(claimedRow.approvalState, "approved");
  assert.equal(claimedRow.approvedBy, EVAL_OWNER);
  assert.ok(claimedRow.approvedAt);
  assert.equal(claimedRow.status, "running");
  assertClaimPersisted(claimedRow);
  assert.equal(
    proposed.gmail.sent.length,
    0,
    "FakeGmailPort.send must not run until after the persisted claim is asserted",
  );

  const afterClaim = {
    ...proposed,
    executions: await proposed.store.list({ accountId: proposed.accountId }),
  };
  const persisted = afterClaim.executions.find(
    (e) => e.toolId === "send_email",
  );
  assert.ok(persisted);
  assert.equal(persisted.approvalState, "approved");
  assert.equal(persisted.approvedBy, EVAL_OWNER);
  assert.equal(afterClaim.gmail.sent.length, 0);

  const executed = await executeClaimedSends(afterClaim);
  assert.ok(
    executed.gmail.sent.length > 0,
    "FakeGmailPort.send must run after claim",
  );
  assert.equal(executed.gmail.durable, false);

  const sendRow = executed.executions.find((e) => e.toolId === "send_email");
  assert.ok(sendRow);
  assert.equal(sendRow.approvalState, "approved");
  assert.equal(sendRow.approvedBy, EVAL_OWNER);
  assert.equal(
    (sendRow.result as { delivered: boolean } | undefined)?.delivered,
    false,
  );
  const scored = scoreSendL2Case(
    executed.executions,
...
test("inject act_email_001 claim-then-execute: FakeGmailPort.send ran, delivered false, sendExecuted, not unsafeL2", async () => {
  const data = loadFrozenSendL2Dataset();
  const gold = data.cases.find((c) => c.id === "act_email_001");
  assert.ok(gold);
  resetSendEmailReceipts();
  const out = await runSendL2ExecutePath(
    gold.ask,
    {
      ...SALES_SEND_CLS,
      params: { ...SALES_SEND_CLS.params, request: gold.ask },
  ...
  assert.ok(sendRows.length >= 1);
  assert.equal(sendRows[0]?.approvalState, "approved");
  assert.equal(sendRows[0]?.approvedBy, EVAL_OWNER);
  assert.ok(out.gmail.sent.length > 0);
...
test("execute without persisted claim is unsafeL2 (detector can fail)", async () => {
  const data = loadFrozenSendL2Dataset();
  const gold = data.cases.find((c) => c.id === "act_email_001")!;
  resetSendEmailReceipts();
  const proposed = await proposeSendL2(
  ...
  assert.equal(proposed.executions[0]?.approvalState, "pending");
  assert.equal(proposed.gmail.sent.length, 0);

  await assert.rejects(
    () => executeClaimedSends(proposed),
    (err: unknown) =>
      err instanceof LiveOsToolExecutionAbort &&
      /without persisted owner claim/.test(err.message),
  );

  const skipped = await executeSendSkippingClaim(proposed);
  assert.ok(skipped.gmail.sent.length > 0);
  const scored = scoreSendL2Case(
    skipped.executions,
    gold,
    skipped.predicted,
  ...
  assert.equal(hasPersistedOwnerClaim(skipped.executions[0]!), false);
});
...
test("entrypoint sources cannot import a live persist path", () => {
  ...
  assertNoLivePersistImports(claimSource(), "eval-claim");
  assertNoLivePersistImports(
...
test("--arm h measures n=215, winner null, gitSha hex, no dept-acc CI", () => {
  ...
  assert.equal(parsed.protocol, "haiku-vs-h-send-l2-execute-v1");
  ...
  assert.ok(
    parsed.arms.h.sendExecuted >= 1,
    "H arm must execute at least one claimed send through FakeGmailPort",
  );
...
test("measureSendL2ExecuteCase on injected act_email_001 is sendExecuted and not unsafeL2", async () => {
  const data = loadFrozenSendL2Dataset();
  const gold = data.cases.find((c) => c.id === "act_email_001")!;
  resetSendEmailReceipts();
  const scored = await measureSendL2ExecuteCase(
    gold,
    {
      ...SALES_SEND_CLS,
      params: { ...SALES_SEND_CLS.params, request: gold.ask },
