...
import { isClarification } from "../agents/_department.ts";
...
test("the resolver refuses asks that are not clearly an action", () => {
  ...
  assert.ok(
    isClarification(ask("Add a note saying she prefers texts.")),
    "no customer named → clarify",
  );
  assert.ok(
    isClarification(ask("Add a note to Sarah Chen's record.")),
    "no note text → clarify",
  );
  const acted = ask(
    "Add a note to Sarah Chen's record saying she prefers texts.",
  );
  assert.ok(acted);
  assert.ok(!isClarification(acted));
});
