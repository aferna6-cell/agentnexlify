...
import { isResolved, resolveByName } from "../../agents/_resolve.ts";
...
/** Resolve a pipeline customer by id, then centralized name resolution. */
export function resolveCustomer(
  ...
  if (!name) return null;
  const resolution = resolveByName(leads, (l) => l.name, name);
  return isResolved(resolution) ? resolution.match : null;
}
