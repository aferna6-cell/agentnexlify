...
test("every department that writes to a customer can propose a send", async () => {
  ...
  const cases: [Agent, string][] = [
    ...
    [
      operations,
      "Email mike.johnson@example.com to confirm his tire rotation appointment.",
    ],
    [invoicing, "Email sarah.chen@example.com about her outstanding invoice."],
    [
      customerService,
      "Email sarah.chen@example.com a reply to her question about our hours.",
    ],
  ];
...
test("no department sends when the owner only asked for words", async () => {
  const cases: [Agent, string][] = [
    [
      sales,
      "Draft an email to sarah.chen@example.com about the quote, I'll look it over first.",
    ],
    [
      operations,
      "Write me something I could send to mike.johnson@example.com about his appointment.",
    ],
    [
      invoicing,
      "What would you say to sarah.chen@example.com about her overdue invoice?",
    ],
    [
      customerService,
      "Rough out a reply I could send to sarah.chen@example.com.",
    ],
  ];
  for (const [dept, ask] of cases) {
    h = harness();
    ...
    assert.deepEqual(
      await executions(),
      [],
      `${dept.agent_id} must not propose a send for: ${ask}`,
    );
  }
});
...
test("a recipient is never invented from a name in the pipeline", async () => {
  ...
  for (const row of await executions()) {
    assert.notEqual(
      row.toolId,
      "send_email",
      "a send was proposed without an address from the owner",
    );
  }
});
...
test("an ambiguous recipient produces a question, not a draft and not a send", async () => {
  ...
  const out = await run(
    sales,
    "Email Mike about the quote we discussed.",
    twoMikes,
  );
  assert.deepEqual(
    await executions(),
    [],
    "nothing may be proposed while the recipient is unknown",
  );
  assert.equal(out.needsClarification, true);
  assert.ok(
    out.orchestratorNotes.join(" ").toLowerCase().includes("mike"),
...
test("a record mutation runs, and the same ask phrased as a question does not", async () => {
  ...
  await run(
    adminRecords,
    "Add a note to Sarah Chen's record saying she prefers morning appointments.",
  );
  ...
  assert.equal(
    acted[0]!.status,
    "succeeded",
    "a level-1 internal write needs no approval",
  );

  h = harness();
  await run(
    adminRecords,
    "Should I be adding notes to customer records when they prefer mornings?",
  );
  assert.deepEqual(
    await executions(),
    [],
    "a question about the practice must never perform it",
  );
});
