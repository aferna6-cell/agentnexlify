...
import { resolveLead, resolvedMatch } from "./_resolve.ts";
  ...
  resolveSkill: ({ ownerAsk, params, context }) => {
    ...
    const name =
      typeof params.customer_name === "string"
        ...
        : "";
    if (!name) return "lead_nurture";
    const openQuoted = context.pipelineLeads.filter(
      (l) =>
        l.quoteAmount &&
    ...
    const lead = resolvedMatch(
      resolveLead({ ...context, pipelineLeads: openQuoted }, name),
    );
