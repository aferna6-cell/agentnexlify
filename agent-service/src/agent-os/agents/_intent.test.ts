...
test("creating a business object and communicating about one are different tasks", () => {
  const pairs: [string, string][] = [
    ["Create a quote for Dana Whitfield, parts $300 and labor $200.", "Email Dana about her existing quote."],
    ["Generate an invoice for the Whitfield job.", "Email Dana about that invoice we sent her."],
    ["Draft a service agreement for new customers.", "Email Dana the service agreement we already agreed."],
    ["Write a job post for a part-time detailer.", "Email the applicant about the job post we ran."],
  ];
  for (const [creating, communicating] of pairs) {
    assert.equal(readAskIntent(creating).intent, "create", creating);
    assert.equal(readAskIntent(communicating).intent, "communicate", communicating);
...
test("a task with no stated channel or permission is ambiguous, not authorized", () => {
  for (const ask of ["Follow up with Priya.", "Circle back with the Whitfield account."]) {
...
test("replying and initiating are different subjects", () => {
  for (const ask of ["Reply to the customer asking about our hours.", "Respond to the question that came in."]) {
    assert.equal(readAskIntent(ask).subjectType, "inbound_message", ask);
  }
  for (const ask of ["Email dana@example.com that the part arrived.", "Text Priya that we're running late."]) {
...
test("a subject is recognised in the plural as well as the singular", () => {
  const singularPlural: [string, string][] = [
    ["Remind him about the appointment.", "Remind them about the appointments."],
  ...
  for (const [one, many] of singularPlural) {
    assert.equal(readAskIntent(one).subjectType, readAskIntent(many).subjectType, `${one} / ${many}`);
  }
});
