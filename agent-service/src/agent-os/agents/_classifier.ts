...
export interface Candidate {
  ...
  confidence: number;
  /**
   * Raw evidence score, before score/(score+2) saturation. Heuristic only.
   * The orchestrator uses this for relative-margin ambiguity so two strong
   * candidates are not labelled a tie merely because confidence saturates.
   */
  score?: number;
}
...
export function classifyHeuristic(ask: string): Classification {
  ...
  // then filter to positives. Semantic score is additive — keywords stay.
