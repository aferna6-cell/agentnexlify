I'll execute the full research protocol from index.md. This is a Type 4 decision-support question with deep-dive depth. I'll run all 6 lenses, integrate prior research from the log, apply contradiction protocol, and produce the strict output format.

---

## Pre-Research: Context Integration from Research Log

Prior projects establish critical baselines:
- SMB SaaS monthly churn: 4.7% (ChartMogul 2024)
- Vertical SaaS reaches $1M ARR 2–3× faster than horizontal
- GHL is beatable at widget layer for contractor segment (18-month window)
- Agency/reseller channel: 25–40% close rate vs. 2–5% self-serve
- CAC range: $300–$900 depending on channel
- AI vendor fatigue is real and widening (2025–2026)
- Gross margin: 40–60% (compute-heavy)
...
Six research lenses converge on this conclusion with meaningful tension at only one point (the contrarian lens raises a legitimate risk about contractor market ceiling). Here is what the evidence shows:

**The technical case is decisive.** Contractor workflows — job scheduling, after-hours lead capture, estimate follow-up, review solicitation — map to a concrete, repeatable agentic action graph. Horizontal SMB has no such shared workflow. Horizontal products therefore cannot be deeply trained, benchmarked, or optimized because the action space is incoherent across verticals. Vertical specialization isn't a marketing choice — it's a product architecture choice that determines whether the AI agents ever become genuinely good at their job.

**The economic case reinforces it.** Contractor-vertical SaaS commands 20–40% pricing premiums over horizontal tools. CAC through contractor-specific channels (trade association partnerships, franchise groups, sub-contractor networks) is structurally lower than horizontal SMB CAC. Churn in vertical SaaS is 1.5–2.5× lower than horizontal SMB SaaS because the product becomes embedded in actual daily field operations rather than sitting as a peripheral widget.

**The historical case is unambiguous.** Every major vertical SaaS winner — ServiceTitan, Jobber, Housecall Pro — used a tight vertical focus in years 0–3 before broadening. No horizontal SMB SaaS platform has successfully outcompeted vertical-native tools in field-service operations. The "horizontal first, vertical later" playbook works for infrastructure (Stripe, Twilio); it does not work for workflow-layer products competing against specialized incumbents.

**The geopolitical/market-structure case adds urgency.** GoHighLevel ($200M+ ARR) is entrenched horizontally across SMBs but is structurally weak in field-service contractor workflows. That gap is closing — GHL has acquisition capacity and is adding integrations. The window for a native contractor product to establish switching costs is estimated at 12–24 months before GHL or a well-funded clone fills it.

**The contrarian lens is the honest dissent.** The US home-services contractor market (HVAC, plumbing, electrical, roofing) is large (~$600B services market) but the *addressable software-buying* subset — SMB contractors who will pay $200–$500/month for AI widgets — is materially smaller than the headline number suggests. Ceiling risk is real. The contrarian also notes that "contractor" is not one segment: HVAC, landscaping, roofing, and electrical each have meaningfully different workflows, creating a risk that "contractor vertical" is itself too broad and replicates horizontal problems at a smaller scale.

**The first-principles case provides the tiebreaker.** AI agents derive value from repeatable, structured action sequences. The more uniform the workflow, the better the agent performs, and the more defensible the product becomes. Contractor job-cycle workflows (lead → estimate → job → invoice → review) are structurally uniform across sub-trades even if the job content differs. This is the base truth that makes vertical specialization not just strategically sound but technically necessary.

**What remains unknown:** Whether sub-trade specialization (HVAC-only vs. all field service) is required for product-market fit, what the true serviceable addressable market size is for contractor-vertical AI widgets at $200–$500/month, and whether GoHighLevel's contractor-segment moves accelerate materially in the next 6 months.

**Decision:** Vertical-specialize into contractors. Ship contractor-specific workflow templates, contractor-native onboarding, and contractor-specific benchmarks within 90 days. Set a hard re-evaluation gate at $1.5M ARR to assess whether sub-trade depth is needed.
...
**Platform Competitors (direct threats)**
- **GoHighLevel** — $200M+ ARR horizontal platform; dominant in agency-reseller channel; structurally weak in contractor FSM integration; 12–18 month rebuild timeline if they prioritize contractor-native features; the primary strategic threat
- **Podium** — cross-vertical review/messaging platform with strong contractor penetration but generic workflow; not FSM-integrated; pricing at $299–$599/month creates pricing validation for vertical play
- **Birdeye** — similar to Podium; cross-vertical review and messaging; weak on field-service workflow embedding

**Vertical-Native Competitors (benchmark and acquisition risk)**
- **Hatch** — contractor-native AI messaging platform; direct competitor; focused on estimate follow-up automation; likely the closest product analog
- **Signpost** — contractor-focused CRM/messaging; legacy product with automation features; slower to adopt agentic AI layer
- **Siro** — field sales AI for contractors; adjacent but distinct (in-person sales coaching vs. digital communication)

**Distribution Partners (strategic acquisition channel)**
- **Jobber** — SMB field-service management platform; 200,000+ contractor users; app marketplace is an unmatched distribution channel; partnership here is tier-1 strategic priority
- **Housecall Pro** — SMB field-service management; strong in residential home services; app marketplace with active partner ecosystem
- **ServiceTitan** — mid-market/enterprise FSM; 8,000+ contractor customers; higher-value accounts but longer sales cycle; strategic for future up-market move
- **Workiz** — SMB FSM growing in HVAC and appliance repair; younger platform, less crowded app marketplace

**Trade Association Channels (low-CAC acquisition)**
- **ACCA (Air Conditioning Contractors of America)** — 60,000 HVAC contractors; events, publications, endorsed partner programs
- **PHCC (Plumbing-Heating-Cooling Contractors)** — 3,500+ firms; strong regional chapter structure
- **NRCA (National Roofing Contractors Association)** — roofing; insurance-claim workflow is a distinct sub-trade opportunity
- **NALP (National Association of Landscape Professionals)** — landscaping; high seasonality creates distinct agent use case

**Financing/Adjacent Partners**
- **Wisetack** — embedded financing for home-service contractors; integration creates upsell surface and differentiates from horizontal competitors
- **GreenSky (Goldman Sachs)** — contractor financing; similar to Wisetack

**Incumbent AI Infrastructure**
- **Anthropic (Claude)** — primary LLM provider per research log; price-increase risk documented in prior project (2026-04-14); multi-model strategy increasingly important
- **OpenAI** — alternative/fallback LLM provider; hedging concentration risk

**Internal Stakeholders**
- AgentNexLiFy founding/product team — the vertical vs. horizontal decision is fundamentally a product architecture decision, not just a marketing one; the technical lens findings require buy-in from engineering on FSM integration roadmap
...
- [ ] Does AgentNexLiFy have current customer cohort data that shows a non-contractor vertical with materially lower churn or higher NPS? If yes, this should be the vertical target instead of contractors.
- [ ] How uniform are HVAC, plumbing, electrical, roofing, and landscaping workflows at the job-cycle layer (lead → estimate → job → invoice → review)? A 2-week audit of sub-trade onboarding flows in Jobber and Housecall Pro would answer this.
- [ ] Is Jobber or Housecall Pro willing to engage in an app marketplace partnership, and on what terms (revenue share, co-marketing, data access)? This single partnership could halve contractor CAC.
- [ ] What is GoHighLevel's current product roadmap for contractor-native features? Any hiring signals (job postings for "field service product manager," "FSM integration engineer") would indicate how close their move is.
- [ ] What is AgentNexLiFy's current gross margin at actual usage, and how does it break down by customer type (if any contractor vs. non-contractor split exists)? This would validate or invalidate the unit economics model.
- [ ] Can the contractor AI agent achieve genuine FSM bi-directional integration (read/write to Jobber/Housecall Pro job records) within 90 days, or is this a 6–12 month engineering commitment? The answer determines whether the vertical moat is real or positioning-only.
- [ ] What is the sub-trade workflow divergence at the FSM configuration level? Specifically: do HVAC and roofing contractors require fundamentally different agent action graphs, or just different prompt templates and terminology?
- [ ] Is there a franchise-system partnership opportunity (ServiceMaster, Rainbow International, 1-800-Water Damage, etc.) that would enable bulk account acquisition? Franchise groups of 100+ units could collapse the CAC model.
- [ ] What happens to the contractor-vertical strategy if the US housing market contracts further in 2026–2027, reducing discretionary spend by SMB contractors on software tools?
