...
- AI: Anthropic Claude (`claude-sonnet-5`, `claude-opus-4-8`, `claude-haiku-4-5-20251001`; legacy-but-valid `claude-sonnet-4-6`/`claude-opus-4-7`)
...
- **Claude Code hooks** — pre-edit sensitive-file warn, post-edit pattern scan, anti-desperation, UltraPlan/UltraThink, 90% confidence gate (`confidence-gate.sh`; escape via `confidence-gate-escape.md` when blocked on humans), 15-msg handoff summary (`scripts/claude-hooks/message-counter.sh`), Opus 4.7 feature reminder (`scripts/claude-hooks/invoke-opus-47-features.sh` — nudges self-verification / /ultrareview / task-budgets / 3x-vision based on prompt keywords), agent-browser router (`scripts/claude-hooks/route-to-agent-browser.sh` — routes WebFetch/WebSearch to agent-browser CLI when installed; falls back to native tools otherwise)
