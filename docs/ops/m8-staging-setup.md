...
4. **Connect Calendar + Gmail** via fresh PKCE-free URLs in `audits/artifacts/m8-oauth-owner-urls.md` (still zero `integrations` rows for `google_calendar`/`gmail` until owner consents after PKCE fix `dfa358d`; re-checked 21:37Z — still 0 callbacks since 21:30Z)
