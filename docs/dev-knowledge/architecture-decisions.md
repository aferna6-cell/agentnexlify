...
### ADR-2026-07-20-001: GitHub Issues as the Cross-Provider Agent Task Hub
**Date:** 2026-07-20
**Decision:** Codex, Fable 5, and Kimi 3 operate as one peer product team under `docs/TEAM_OPERATING_CONTRACT.md`. One GitHub issue owns each outcome. Structured issue comments generated by `scripts/teamctl.py` carry claims, leases, updates, handoffs, peer reviews, and local proof. Active lanes use isolated `team/<issue>/<agent>/<lane>` branches/worktrees. Ordinary changes require one non-author approval; risky reversible changes require both other agents plus rollback protection and the full local gate. Team commits include `[skip ci]`, and team pull-request jobs have head-branch guards, so coordination and verification allocate no GitHub Actions runner minutes.
**Why:** Provider-private scratch files and independent task execution create duplicated work, stale context, and integration conflicts. GitHub Issues provide a provider-neutral durable event stream without hosted runner usage. Exclusive lane leases prevent overlap, peer quorum removes routine owner dependency, and Tier C safe-substitute rules preserve autonomy without authorizing irreversible external actions.

### ADR-2026-07-21-001: Photo-Quote Uses Tiered Severity + Per-Vertical Confidence Floors
**Date:** 2026-07-21
**Decision:** The photo-quote feature prices a job from one photo as a single severity tier (`minor`/`major`/`needs_human`) with a low-high dollar range, not a point estimate. Each of the six verticals (plumbing, roofing, hvac, auto_body, landscaping, pest) has its own confidence floor (0.7 / 0.8 / 0.75 / 0.75 / 0.7 / 0.7); a quote whose confidence falls below the floor is forced to `needs_human` and its price zeroed. Floors are overridable per tenant via `tenant_pricing_rules.min_confidence_threshold`; an unknown vertical falls back to 0.75. Pricing bands live in `tenant_pricing_rules.rules_jsonb` (`{damage_type: {minor/major: {low, high}}}`), seeded from HomeAdvisor/Angi 2025 averages, and are injected verbatim into the vision prompt so every returned price is grounded in a configured band. `quote_requests` is `client_id`-scoped (migration 108).
**Why:** A single-photo estimate is inherently uncertain, and uncertainty varies by trade — roof damage is far harder to price from a ground-level phone photo than a visible plumbing leak, so roofing carries the highest floor and routes to human review more often by design. A point estimate would imply false precision and expose the tenant to disputes; a range plus an explicit `needs_human` handoff protects the tenant's credibility by declining to quote what a person should quote. Grounding prices in the tenant's own bands (never an invented number) is the same moat the widget chat uses. Billing is metered (500/mo included, $0.15 overage) and fail-open, so an unconfigured Stripe SKU counts usage locally without blocking visitors. Spec: `specs/photo-quote_spec.md`. KB: `knowledge-base/wiki/widget/photo-quote.md`.

### ADR-2026-07-22-001: Drive-KB Uses a Multi-Provider `tenant_integrations` Row, Not Drive-Specific Columns
**Date:** 2026-07-22
**Decision:** The Google Drive knowledge-base connection stores its OAuth tokens, selected folder, and enabled flag on a `tenant_integrations` row keyed by a `provider` field (`drive` today), rather than adding Drive-specific columns to the `tenants` table. The daily sync routine (`backend/services/drive_kb_sync.py`, `run_drive_kb_sync_due`) dispatches by provider, and the read-side surface (`backend/routers/kb_integrations.py`) is provider-neutral. Adding Dropbox, OneDrive, or Box (post-GA, v1.1) is a new provider client module plus an enum value, not a schema migration or a parallel sync pipeline. Drive scope is OAuth `drive.readonly`, one folder per tenant; sync is daily + diff-based with advisory regex PII flagging (`integration_sync_log.files_pii_flagged`) and tier-gated document limits (Free 10 / Growth 100 / Pro unlimited). While a folder is actively syncing, the Knowledge dashboard is read-only (manual upload/remove disabled) so manual edits are not clobbered by the next sync.
**Why:** A single-provider design (Drive columns on `tenants`) would force a schema migration and a forked sync path for every additional provider, and the spec (Resolved Q1 B) commits to multi-provider on demand. Modeling the connection as one row per `(tenant, provider)` keeps the abstraction open from day one at no extra cost to the Drive-only v1, while the "ship one provider at a time, gated on Drive holding a low error rate in production" rule (issue #55) prevents the schema flexibility from turning into premature multi-provider surface. Advisory (non-blocking) PII flagging is chosen over silent removal because a false positive that quietly dropped a real price list would be worse than a visible warning the owner can act on. Spec: `specs/drive-kb-onboarding_spec.md`. KB: `knowledge-base/wiki/integrations/drive-kb-onboarding.md`. Runbook: `docs/runbooks/drive-kb-failures.md`.
