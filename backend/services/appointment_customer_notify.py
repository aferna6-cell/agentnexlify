...
Design contract (same as the other best-effort notification modules):
non-blocking, demo-safe (``send_sms``/``send_email`` no-op for demo tenants),
gated on the customer having an email/phone, and every entry point NEVER raises
— callers schedule these as background tasks inside the update response cycle.
    ...
    try:
        ...
        if customer_email:
            try:
                ...
                logger.warning(
                    "appointment_customer_notify: reschedule email FAILED to %s",
                    customer_email,
                    exc_info=True,
                )
        ...
        if customer_phone:
            try:
                ...
                await send_sms(to=customer_phone, body=sms_body, tenant_id=tenant_id)
            except Exception:
                logger.warning(
                    "appointment_customer_notify: reschedule SMS FAILED to %s",
                    customer_phone,
                    exc_info=True,
                )
    except Exception:
    ...
    try:
        ...
        if customer_email:
            try:
                ...
                logger.warning(
                    "appointment_customer_notify: cancel email FAILED to %s",
                    customer_email,
                    exc_info=True,
                )
        ...
        if customer_phone:
            try:
                ...
                logger.warning(
                    "appointment_customer_notify: cancel SMS FAILED to %s",
                    customer_phone,
                    exc_info=True,
                )
    except Exception:
