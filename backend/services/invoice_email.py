"""Invoice email rendering (issue #473 split). Moved verbatim from
routers/invoices.py — HTML bodies for single-send and bulk-send emails.
...
def build_bulk_invoice_email_html(inv_num: str, lead: dict, biz_name: str, total: float, payment_link: str | None) -> str:
    """Compact bulk-send email body (moved verbatim from the bulk-send loop)."""
    ...
    return f"<div style='font-family:sans-serif;max-width:600px;margin:0 auto;'><h2>Invoice {inv_num}</h2><p>Hi {lead.get('name', 'there')},</p><p>You have an invoice from <strong>{biz_name}</strong> for <strong>${total:,.2f}</strong>.</p>{link_html}<p style='color:#6b7280;font-size:12px;margin-top:24px;'>-- {biz_name}</p></div>"
