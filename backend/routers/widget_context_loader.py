"""Widget chat context loader — DB queries and system-prompt assembly.

Extracted from widget_chat.py — single concern: load all per-tenant
context data needed to build the LLM system prompt for a chat turn.

Public API:
    load_chat_context(...)  -> ChatContext
"""
...
@dataclass
class ChatContext:
    """All context data loaded for a single chat turn."""
    ...
    faq_data: list[dict[str, Any]] = field(default_factory=list)
    bh_data: dict[str, Any] | None = None
...
def load_chat_context(
    *,
    ...
    """Load all tenant context and assemble the LLM system prompt.

    Executes synchronously (Supabase client is sync). All DB calls are
    individually guarded so a single query failure degrades gracefully
    rather than failing the entire chat turn.

    Returns a ChatContext with everything the main chat handler needs.
    """
    tid = tenant["id"]
    db = get_service_supabase()
    context_started = perf_counter()

    intent_window = _build_intent_window(message, messages)
    ...
    # --- Website content (fallback when no knowledge_base) ---
    ...
    # --- Menu items (restaurant vertical) ---
    menu_items = None
    ...
    # --- Job listings (jobs intent detected) ---
    job_listings = None
    ...
    # --- Bid templates (contractor bid intent detected) ---
    bid_templates = None
    ...
    # --- Custom lead field definitions (used in bid context) ---
    custom_field_defs = []
    if needs_bid_context:
        try:
            cf_result = (
