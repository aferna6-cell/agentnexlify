...
async def track_email_open(
    ...
    sig: str = Query("", description="HMAC signature", max_length=64),
):
    """Log an email open event and return a 1x1 tracking pixel.

    The insert is gated on a valid HMAC bound to (tid, lid, eid). Without it,
    anyone who guesses the URL shape could inject open events into any tenant's
    email_events under the service-role key. An invalid/missing signature still
    returns the pixel (so attackers can't probe validity) but skips the write.
    """
    expected = _make_tracking_sig(tid, lid, eid)
    if sig and hmac.compare_digest(sig, expected):
        ...
        logger.debug("Email open tracking: missing/invalid signature for tid=%s", tid)
