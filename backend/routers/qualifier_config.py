"""Per-tenant AI lead-qualifier settings (migration 147).
...
columns not existing yet (safe to deploy before/with migration 147):
a missing-column error falls back to the safe defaults
``(enabled=True, min_intent=0)`` that match pre-147 behavior.
"""
