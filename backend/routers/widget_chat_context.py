"""System-prompt context assembly for the widget chat pipeline (issue #472).

Loads every tenant-scoped grounding source (FAQs, hours, corrections,
website content, menu, jobs, bid templates, custom fields, chat flow),
builds the system prompt, and returns a `ChatContext` the route hands to
the Claude call. Moved verbatim from the widget_chat route body.

The per-message KB article retrieval stays in widget_chat.py (its call
site is a pinned test patch point); the resulting refs are passed in.
...
@dataclass
class ChatContext:
    """Everything the LLM call needs from the context-build stage."""
...
def build_chat_context(
    *,
    ...
    """Stage 6 — build the system prompt with compact, intent-aware context."""
    tid = tenant["id"]
