...
async def _run_new_lead_followups(
    ...
    try:
        ...
        logger.info(
            "SMS_TRIGGER[/lead]: trigger_sequence completed for lead %s", lead_id
        )
    except Exception:
        logger.warning(
    ...
    try:
        ...
        logger.warning(
            "Failed to enroll lead %s in email sequences", lead_id, exc_info=True
        )
...
async def submit_lead(
    request: Request, req: WidgetLeadRequest, background_tasks: BackgroundTasks
):
    ...
    if not lead_id:
        ...
        try:
            ...
            logger.debug(
                "lead_submit: conversation_id %r is not a UUID, omitting",
                conversation_id,
            )
    ...
    if lead_id:
        background_tasks.add_task(score_lead_background, lead_id, tenant["id"])
...
async def submit_offline_contact(
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "offline_contact: dedup check failed: %s",
                dedup_err,
                exc_info=True,
            )
        ...
        if not lead_id:
            ...
            try:
                ...
                logger.error(
                    "offline_contact: lead INSERT failed: %s",
                    insert_err,
                    exc_info=True,
                )
        ...
        if lead_id:
            ...
            try:
                ...
                logger.warning(
                    "offline_contact: fire_event_background failed",
                    exc_info=True,
                )

            # Score the lead (non-blocking via BackgroundTasks)
            background_tasks.add_task(score_lead_background, lead_id, tenant_id)
