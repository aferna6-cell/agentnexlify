"""Widget chat second-tier managed-agent fallback.

Extracted from widget_chat.py to keep the route handler focused.
Tests patch `backend.routers.widget_chat._run_support_fallback`
and `backend.routers.widget_chat.FALLBACK_MARKER`, so these stay
re-exported from widget_chat.py.
"""
...
async def _run_support_fallback(
    *,
    ...
    try:
        ...
        # Look up log_activity via the widget_chat module so tests can patch
        # `backend.routers.widget_chat.log_activity` and have the patch flow
        # through to this helper. Direct import would freeze the binding at
        # module-load time and tests would see the unpatched original.
        try:
