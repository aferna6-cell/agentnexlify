# M8 Confidence Gate — 20260831T211851Z

**Score:** 76%  
**Verdict:** HOLD  
**Reason:** Google device step-up still blocks consent; Calendar+Gmail disconnected; zero `/callback` hits since 21:15Z

## Recheck since 21:15Z

| Check | Result |
|-------|--------|
| Staging health | ok (deployment `0249c2b4` / `d235979`) |
| `GET .../google/status` | `connected=false` |
| `GET .../gmail/status` | `connected=false` |
| `integrations` rows (tenant) | **0** |
| `tenant_integrations` rows | **0** |
| Railway HTTP `/google/callback` + `/gmail/callback` since 21:15Z | **0** |
| Fresh auth URLs | **no `code_challenge`** (minted `20260831T211851Z`, expire `2026-08-31T22:18:51+00:00`) |
