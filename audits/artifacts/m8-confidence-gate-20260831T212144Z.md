# M8 Confidence Gate — 20260831T212144Z

**Score:** 76%  
**Verdict:** HOLD  
**Reason:** Google device step-up still blocks consent; Calendar+Gmail disconnected; zero `/callback` hits since 21:18Z

## Recheck since 21:18Z

| Check | Result |
|-------|--------|
| Staging health | ok (deployment `83d460aa` / `fedee9b`) |
| `GET .../google/status` | `connected=false` |
| `GET .../gmail/status` | `connected=false` |
| `integrations` rows (tenant) | **0** |
| `tenant_integrations` rows | **0** |
| Railway HTTP `/google/callback` + `/gmail/callback` since 21:18Z | **0** |
| Fresh auth URLs | **no `code_challenge`** (minted `20260831T212144Z`, expire `2026-08-31T22:21:44+00:00`) |
