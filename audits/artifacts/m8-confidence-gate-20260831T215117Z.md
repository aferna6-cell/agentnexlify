# M8 Confidence Gate — 20260831T215117Z

**Score:** 76%  
**Verdict:** HOLD  
**Reason:** Google device step-up still blocks consent; Calendar+Gmail disconnected; zero `/callback` hits since 21:48Z

## Recheck since 21:48Z

| Check | Result |
|-------|--------|
| Staging health | ok (deployment `b3cc5abb-14f8-4012-b9ed-8f78ec5dfa12` / `b1d921e`) |
| `GET .../google/status` | `connected=false` |
| `GET .../gmail/status` | `connected=false` |
| `integrations` rows (tenant) | **0** |
| `tenant_integrations` rows | **0** |
| Railway HTTP `/google/callback` + `/gmail/callback` since 21:48Z | **0** |
| Fresh auth URLs | **no `code_challenge`** (minted `20260831T215117Z`, expire `2026-08-31T22:51:36+00:00`) |
