...
def embedding_predictions(asks: list[str]) -> list[Prediction] | None:
    artifact = ARTIFACTS / "embedding_lr.joblib"
    if not artifact.exists():
        return None
    from train_embeddings import _encoder, predictions_from
    return predictions_from(joblib.load(artifact), _encoder(), asks)
...
def build_architectures(
    ...
    # C2 — optional when an embedding artifact exists.
    if emb is not None:
...
def main() -> None:
    ...
    ap.add_argument("--validation-version", default="v3", choices=["v1", "v2", "v3"])
    ...
    if emb is None:
        print("embedding   skipped (no embedding_lr.joblib artifact)")
    else:
