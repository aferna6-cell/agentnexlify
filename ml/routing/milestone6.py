...
What it does, in order:
  ...
  1. Score every base router on the split (heuristic, TF-IDF, embeddings,
     and Haiku where a credential exists).
...
def build_architectures(
    ...
    # C2 — the same shape as C with the embedding model in the fallback slot.
    # Added because the measurement demanded it, not because the brief listed
    # it: on this split the embedding router is the strongest thing available in
    # the low-evidence region, which is the region the cascade exists to serve.
    # Leaving it out would have meant comparing C against a fallback already
    # known to be weaker there.
    record("C2", "C2: heuristic -> embedding",
           Cascade("C2", [
               Stage("heuristic", heur, heuristic_accepts(floor), "heuristic_below_floor",
                     calibrated=cal.get("heuristic")),
               Stage("embedding", emb, always_accepts, "embedding_had_no_answer"),
           ]))
...
def main() -> None:
    ...
    t0 = time.perf_counter(); emb = embedding_predictions(split.asks)
    print(f"embedding   {time.perf_counter() - t0:5.1f}s")
    ...
    base = {"heuristic": heur, "tfidf": tfidf, "embedding": emb}
    if llm is not None:
        base["llm"] = llm
    ...
    # --- Architectures ------------------------------------------------------
    reports, profiles = build_architectures(labels, heur, tfidf, emb, llm, chosen_values, usd_per_call)
