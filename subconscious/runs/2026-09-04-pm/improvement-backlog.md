...
## Active
- **Step 9M: Nightly AI usage metering coverage sweep** — grep backend/routers/ for Claude API calls lacking usage guard, file ai-ready issues per unguarded file. Autonomous-executable via SKILL.md-edit channel. Confidence: HIGH.

## Parking Lot (survived debate, not chosen)
- **Step 9L: Unapplied migration alerter** — run check_schema_log_migrations.py in nightly, alert if schema-log/migration drift found. Blocked until PR #788 merges. Evidence: f72a274 shows manual schema-log updates bypassing PR flow. Unblock condition: PR #788 merged.
...
- **Step 9L via PR #782** — prior draft subconscious PR (2026-09-03) references this. Absorb into Step 9L parking lot entry above.
...
1. Did Step 9M fire in the first nightly post-implementation? How many unguarded files found?
2. Has PR #788 (check_schema_log_migrations.py) been merged, unblocking Step 9L?
