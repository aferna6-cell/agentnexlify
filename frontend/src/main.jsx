...
const ClientPortalPublicPage = lazy(
  () => import("./pages/ClientPortalPublicPage"),
);
...
const CALENDLY_URL =
  "https://calendly.com/aidanfernandes31/15-minute-agent-nexliffy-demo";
...
/* Handles /onboarding route.
   - With ?step= param (Stripe return): renders the wizard directly.
   - Authenticated, no step param: redirect to /dashboard.
...
function OnboardingRedirect() {
  ...
  // Stripe return (?step=6&session_id=...) — render the wizard, don't redirect
  if (user && hasStepParam) return <OnboardingWizardPage />;

  // Authenticated, no step param → dashboard
  if (user) return <Navigate to="/dashboard" replace />;

  // Not authenticated — if step param present, save destination and go to signup
  if (hasStepParam) {
    sessionStorage.setItem("anx_post_login_redirect", window.location.href);
  }
        ...
        <BrowserRouter>
          <Suspense fallback={<PageLoader />}>
            <Routes>
              ...
              <Route
                path="/medical-office-chatbot"
                element={<MedicalOfficeChatbot />}
              />
              <Route
                path="/restaurant-chatbot"
                element={<RestaurantChatbot />}
              />
              ...
              />
              <Route
                path="/livechat-alternative"
                element={<LiveChatAlternative />}
              />
              <Route path="/tidio-alternative" element={<TidioAlternative />} />
              {/* Marketing paths — scroll to Home anchor sections */}
              <Route
                path="/pricing"
                element={<HomeSection anchor="pricing" />}
              />
              <Route
                path="/features"
                element={<HomeSection anchor="features" />}
              />
              <Route
                path="/about"
                element={<HomeSection anchor="about-us" />}
              />
              <Route
                path="/demo"
                element={<RedirectExternal url={CALENDLY_URL} />}
              />
