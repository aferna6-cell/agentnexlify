...
const modalStyle = {
  ...
  padding: "28px 32px", width: "90%", maxWidth: 900, maxHeight: "90vh",
  overflowY: "auto", border: "1px solid var(--border)",
};

const TEST_TYPES = [
  { key: "subject_line", label: "Subject Line", desc: "Test different email subject lines" },
  { key: "send_time", label: "Send Time", desc: "Test different send times for same content" },
  { key: "body_content", label: "Body Content", desc: "Test different email body copy" },
  { key: "campaign_variant", label: "Campaign Variant", desc: "Fully different campaigns" },
];
...
function EmptyState({ message, icon }) {
  return (
    <div style={{ ...cardStyle, textAlign: "center", padding: "60px 20px" }}>
      ...
      <h3 style={{ margin: "0 0 8px", color: "var(--text-primary)" }}>{message}</h3>
      <p style={{ color: "var(--text-secondary)", margin: 0 }}>Create your first A/B test to get started</p>
    </div>
  );
}
...
function CreateTestModal({ tenantId, token, onClose, onCreated }) {
  ...
  const [variants, setVariants] = useState([
    { name: "Variant A", subject: "", body: "", send_time_override: "", allocation_percent: 50 },
    { name: "Variant B", subject: "", body: "", send_time_override: "", allocation_percent: 50 },
  ]);
  ...
  const addVariant = () => {
    ...
    setVariants((prev) => prev.map((v, i) => ({
      ...v,
      allocation_percent: i < prev.length - 1 ? Math.floor(100 / prev.length) : 100 - prev.slice(0, prev.length - 1).reduce((s, x) => s + x.allocation_percent, 0),
    })));
  };
  ...
  const handleSubmit = async () => {
    ...
    const totalAllocation = variants.reduce((s, v) => s + (v.allocation_percent || 0), 0);
    if (totalAllocation !== 100) { setError("Variant allocations must sum to 100%"); return; }
    setSubmitting(true);
    setError(null);
    try {
      ...
      <div onClick={(e) => e.stopPropagation()} style={modalStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)}
              placeholder="e.g., Summer Subject Line Test" style={inputStyle} />
          </div>
          <div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input type="text" value={description} onChange={(e) => setDescription(e.target.value)}
            placeholder="What are you testing and why?" style={inputStyle} />
        </div>

        <div style={{ marginBottom: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <label style={{ ...labelStyle, margin: 0 }}>Variants ({variants.length}/4)</label>
            {variants.length < 4 && (
              <button onClick={addVariant} style={{ ...btnSecondary, padding: "4px 12px", fontSize: "0.8rem" }}>
                + Add Variant
              </button>
            )}
          </div>
        ...
        {variants.map((v, idx) => (
          <div key={idx} style={{
            ...
            {testType === "subject_line" && (
              <input type="text" value={v.subject} onChange={(e) => updateVariant(idx, "subject", e.target.value)}
                placeholder="Subject line for this variant" style={{ ...inputStyle, marginBottom: 8 }} />
            )}

            {testType === "body_content" && (
              <textarea value={v.body} onChange={(e) => updateVariant(idx, "body", e.target.value)}
                placeholder="Email body for this variant" rows={4}
                style={{ ...inputStyle, resize: "vertical", marginBottom: 8 }} />
            )}

            {testType === "send_time" && (
              <input type="time" value={v.send_time_override} onChange={(e) => updateVariant(idx, "send_time_override", e.target.value)}
                style={inputStyle} />
            )}

            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
              <label style={{ fontSize: "0.8rem", color: "var(--text-secondary)" }}>Allocation:</label>
              <input type="number" value={v.allocation_percent} min={1} max={99}
                onChange={(e) => updateVariant(idx, "allocation_percent", parseInt(e.target.value) || 0)}
        ...
        <div style={{ marginBottom: 16 }}>
          <span style={{ fontSize: "0.85rem", color: "var(--text-secondary)" }}>
            Total allocation: {variants.reduce((s, v) => s + (v.allocation_percent || 0), 0)}%
            {variants.reduce((s, v) => s + (v.allocation_percent || 0), 0) !== 100 && (
              <span style={{ color: "#f87171", marginLeft: 8 }}>(must equal 100%)</span>
            )}
          </span>
        </div>
  ...
  return (
    <div style={{ ...cardStyle, marginTop: 24 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: "1.2rem", color: "var(--text-primary)" }}>Results: {test.name}</h2>
        ...
        <>
          {results.winner && (
            <div style={{
              ...
              Winner: {results.winner.name} (confidence: {results.winner.confidence ? `${(results.winner.confidence * 100).toFixed(1)}%` : "N/A"})
            </div>
          )}
...
export default function ABTestsPage() {
  ...
  const handleStartTest = async (testId) => {
    try {
      await apiFetch(`/ab-tests/${user.tenantId}/${testId}/start`, token, { method: "POST" });
      loadTests();
    } catch (e) {
  ...
  const handleDelete = async (testId) => {
    ...
    try {
      await apiFetch(`/ab-tests/${user.tenantId}/${testId}`, token, { method: "DELETE" });
    ...
    <div style={{ padding: "24px 32px", maxWidth: 1200 }}>
      ...
      {tests.length === 0 ? (
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{
            ...
            <thead>
              <tr>
                {["Name", "Type", "Status", "Variants", "Started", "Actions"].map((h) => (
                  <th key={h} style={{
            ...
            <tbody>
              {tests.map((t) => (
                <tr key={t.id} onClick={() => setSelectedTest(t)}
                  ...
                  <td style={{ padding: "12px 16px", color: "var(--text-secondary)" }}>
                    {t.variants?.length || t.variant_count || (t.ab_test_variants?.length || 0)}
                  </td>
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    <div style={{ display: "flex", gap: 6 }}>
                      {t.status === "draft" && (
                        <button onClick={(e) => { e.stopPropagation(); handleStartTest(t.id); }}
                          style={{ ...btnPrimary, padding: "4px 10px", fontSize: "0.75rem" }}>
                          Start
                        </button>
                      )}
                      {t.status === "running" && (
                        <button onClick={(e) => { e.stopPropagation(); handleCompleteTest(t.id); }}
                          style={{ ...btnSecondary, padding: "4px 10px", fontSize: "0.75rem" }}>
                          Complete
                        </button>
                      )}
                      <button onClick={(e) => { e.stopPropagation(); handleDelete(t.id); }}
                        style={{ background: "none", border: "1px solid var(--border)", color: "var(--text-muted)", padding: "4px 10px", borderRadius: 6, cursor: "pointer", fontSize: "0.75rem" }}>
                        Delete
                      </button>
                    </div>
                  </td>
      ...
      {showCreate && (
        <CreateTestModal
          ...
          onCreated={() => { setShowCreate(false); loadTests(); }}
        />
      )}
    </div>
  );
