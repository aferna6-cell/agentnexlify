...
const COLORS = ["var(--accent)", "var(--green)", "var(--purple, #8b5cf6)", "var(--yellow, #f59e0b)", "#f87171"];
...
export default function MarketingDashboardPage() {
  ...
  const [stats, setStats] = useState({ total_campaigns: 0, total_sent: 0, avg_open_rate: null, avg_click_rate: null });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [dashRes, trendsRes] = await Promise.all([
        apiFetch(`/marketing/${user.tenantId}/dashboard`, token).catch(() => null),
        apiFetch(`/marketing/${user.tenantId}/trends?period=${period}`, token).catch(() => null),
      ]);
      ...
      if (dashRes) {
        ...
        if (dashRes.top_performing_campaign) {
          setTopCampaigns(Array.isArray(dashRes.top_performing_campaign)
  ...
  return (
    <div style={{ padding: "24px 32px", maxWidth: 1400 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {["30d", "90d"].map((p) => (
            <button key={p} onClick={() => setPeriod(p)} style={{
              ...
              background: period === p ? "var(--accent-dim)" : "var(--bg-primary)",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
        {[
          { label: "Total Campaigns", value: stats.total_campaigns, color: "var(--accent)" },
          { label: "Total Sent", value: stats.total_sent.toLocaleString(), color: "var(--green)" },
          { label: "Avg Open Rate", value: openRate, color: "var(--purple, #8b5cf6)" },
          { label: "Avg Click Rate", value: clickRate, color: "var(--yellow, #f59e0b)" },
        ].map((s) => (
        ...
        {topCampaigns.length > 0 ? (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0 }}>
              <thead>
                <tr>
                  {["Campaign", "Type", "Sent", "Open Rate", "Click Rate", "Sent Date"].map((h) => (
                    <th key={h} style={{
                  ...
                  <tr key={c.id || i} style={{ transition: "background 0.15s" }}
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <span style={{
                        ...
                        color: c.type === "email" ? "var(--accent)" : "var(--green)",
                        background: c.type === "email" ? "var(--accent-dim)" : "var(--green-dim)",
                      }}>{c.type?.toUpperCase() || "EMAIL"}</span>
                    </td>
