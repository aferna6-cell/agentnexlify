#!/bin/bash
# Confidence Gate — blocks the FIRST stop to force self-assessment.
# Second stop (after Claude self-assessed) passes through.
# Uses a flag file to prevent infinite block loops.

# Drain stdin
cat < /dev/stdin > /dev/null 2>&1

FLAG="/tmp/.claude-confidence-gate-$$"

# If flag exists, Claude already self-assessed — let it stop
if [ -f "$FLAG" ]; then
    rm -f "$FLAG"
    exit 0
fi

# First stop — create flag and block with confidence prompt
touch "$FLAG"
# Auto-clean flag after 5 minutes in case session dies
(sleep 300 && rm -f "$FLAG") &>/dev/null &

cat << 'GATE'
{"decision":"block","reason":"CONFIDENCE GATE: Before finishing, rate your confidence (0-100%) that this work is correct, complete, and production-ready. Consider: (1) Did you verify the fix works? (tests pass, build clean) (2) Did you check for regressions? (3) Did you handle edge cases? (4) Would a staff engineer approve this? If confidence >= 90%, state your score and finish. If < 90%, state what's uncertain and keep working."}
GATE
