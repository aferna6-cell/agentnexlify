...
- Generate Voyage AI embedding (voyage-3-lite, 512-dim) via Supabase MCP and upsert into `kb_articles` table. Real schema (see migrations/081-kb-articles-and-sources.sql + migrations/111_kb_article_provenance.sql): `slug UNIQUE, title, category, summary, content, embedding vector(512), source_urls TEXT[], tags TEXT[], word_count INT, source_url TEXT, last_validated TIMESTAMPTZ, citation_count INT`. When upserting, ALSO set `source_url` = the `source_url` field from the raw file frontmatter (or empty string if absent), and `last_validated` = current UTC timestamp (ISO 8601). Use `INSERT ... ON CONFLICT (slug) DO UPDATE SET ..., source_url = EXCLUDED.source_url, last_validated = now()` so every successful compile stamps the article as freshly validated. If Supabase MCP is unreachable, skip embedding but continue with markdown compile — log embedding_errors=N.
