...
9J. (Dependabot Auto-Merge) Merge CI-green Dependabot PRs with no review requests:
    1. **List open Dependabot PRs:**
       ...
       (Note: `list_pull_requests` with creator filter is unreliable for bot PRs in headless sessions — use search API instead.)
...
9K. (Stale Subconscious Draft PR Audit) Detect accumulating unreviewed subconscious improvement PRs:
    ...
    5. Threshold logic:
       ...
       c. Additionally, if `stale_count >= 5` OR `critical_count >= 1`:
          ...
          - Post comment via `mcp__github__add_issue_comment` on that PR:
            Body: "Subconscious PR audit (Step 9K): This PR is {age_days} days old. {stale_count} stale subconscious draft PRs (>30d) are open. Please review, merge, or close to prevent backlog accumulation."
    6. Add to nightly summary: "Step 9K: {total_count} subconscious PRs open ({stale_count} stale, {critical_count} critical)"
